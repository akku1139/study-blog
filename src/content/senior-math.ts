      ...
      lessons: [
        {
          ...
          blocks: [
            {"type": "heading", "level": 3, "content": "軸からの距離で、端点と頂点を比べる"},
            {"type": "text", "content": "同じ放物線でも、見る範囲を変えると最も低い点は変わります。坂道の底が柵の外なら、柵の中で最も低いのは入口のどちらかです。定義域付きの最大・最小では、まず「頂点まで行けるか」を確認します。"},
            {"type": "text", "content": "y=a(x−p)²+q の二乗部分は、xと軸pとの距離の二乗です。a>0なら近いほど値が小さく、a<0なら近いほど値が大きい。閉区間では、軸が区間内にあるときだけ頂点を候補に加え、両端の値と比較すれば済みます。"},
            {"type": "text", "content": "軸が動く問題も距離で考えます。区間[0,4]なら両端から等距離の軸はp=2。a>0の放物線の最大値は、p<2なら右端、p>2なら左端にあり、p=2なら両端で同じです。これは値の大小を式で比較した結果とも一致します。"},
            {"type": "note", "variant": "warn", "content": "頂点のy座標がいつでも最小値になるわけではありません。頂点が定義域の外なら候補から外します。また「最大値」と「最大値をとるx」は別の量なので両方を区別して答えましょう。"},
            {"type": "example", "title": "例題：頂点が範囲の外にあるとき", "body": "y=(x−3)²+2、0≤x≤2の最大値と最小値を求めよ。", "answer": "軸3に最も近いのはx=2、最も遠いのはx=0。最小値は3（x=2）、最大値は11（x=0）。頂点の値2はこの定義域ではとれません。"},
            {"type": "example", "title": "例題：軸が動くと最大値はどちらの端か", "body": "y=(x−p)²、0≤x≤4の最大値をpで場合分けせよ。", "answer": "端点の値はp²と(4−p)²。右端−左端=16−8pなので、p<2では(4−p)²（x=4）、p>2ではp²（x=0）。p=2では最大値4を両端でとります。"},
            {"type": "practice", "title": "練習：判断の理由も確かめよう", "problems": [{"body": "y=−(x−1)²+5、−1≤x≤2の最大・最小は。", "answer": "軸が区間内なので最大値5（x=1）。軸から遠い端は−1なので最小値1（x=−1）。"}, {"body": "y=(x+2)²−1、0≤x≤3の最大・最小は。", "answer": "軸−2は範囲外。近いx=0で最小値3、遠いx=3で最大値24。"}, {"body": "y=(x−p)²、0≤x≤4で最小値が0となるpの範囲は。", "answer": "0≤p≤4。二乗が0となるx=pを、定義域内で選べることが必要十分です。"}]},
        ...
        },
        {
          ...
          blocks: [
            {"type": "heading", "level": 3, "content": "不等式は「どの高さまで許すか」を数直線に戻す"},
            {"type": "text", "content": "方程式は境界の点を探す問題、不等式はその境界の間や外側を選ぶ問題です。たとえば放物線の高さが2以下の場所を探すなら、まず高さ2の水平線との交点を探すと、許される区間が見えてきます。"},
            {"type": "text", "content": "右辺が0でなければ移項して一つの二次式にまとめます。異なる根があるときは根で区切った各区間から一点ずつ代入すれば符号を確認できます。因数の符号は根を通らない限り変わらないので、代表一点による確認が有効なのです。"},
            {"type": "text", "content": "複数の条件を同時に満たす解は共通部分です。「または」なら和集合、「かつ」なら共通部分を数直線で読みます。境界を含むかは元の不等号で決め、最後に境界を代入して検算しましょう。"},
            {"type": "note", "variant": "warn", "content": "未知数xを含む式で両辺を割ると、符号による場合分けや0の確認が必要になります。特に x(x−2)>0 を x−2>0 にしてはいけません。負×負で正になる解を落としてしまいます。"},
            {"type": "example", "title": "例題：水平線との比較", "body": "x²−4x+3≤2を解け。", "answer": "(x−2)²≤3なので、2からの距離が√3以下。解は2−√3≤x≤2+√3。移項後の二次式の根を求めても同じです。"},
            {"type": "example", "title": "例題：定義域との共通部分", "body": "x²−x−6>0、かつ−3≤x≤2を満たす範囲は。", "answer": "(x+2)(x−3)>0よりx<−2またはx>3。指定区間との共通部分は−3≤x<−2。−3は元の式で6>0、−2は0なので含みません。"},
            {"type": "practice", "title": "練習：判断の理由も確かめよう", "problems": [{"body": "x(x−2)>0の解は。", "answer": "x<0またはx>2。x<0では両因子とも負なので積は正です。"}, {"body": "−(x+1)²≥0の解は。", "answer": "x=−1のみ。左辺は常に0以下で、0になるときだけ条件を満たします。"}, {"body": "x²+4x+5≤1の解は。", "answer": "(x+2)²+1≤1よりx=−2のみ。平方完成で最小値と等号条件がわかります。"}]},
      ...
      lessons: [
        {
          ...
          blocks: [
            {"type": "heading", "level": 3, "content": "式を変える前に、符号と定義できる範囲を見る"},
            {"type": "text", "content": "根号は「二乗すると元に戻る数のうち、0以上のもの」を選ぶ記号です。だから√(x²)はxそのものではなく|x|。温度が−3でも原点からの距離は3という数直線の感覚が、計算の符号ミスを防ぎます。"},
            {"type": "text", "content": "絶対値が二つある式は、中身が0になる点をすべて数直線に打ちます。点と点の間では各中身の符号が一定なので、一つの区間ごとに絶対値を外せます。境界でどちらの式からも同じ値になることも確かめます。"},
            {"type": "text", "content": "有理化では、和と差の積を利用すると根号が消えます。ただし分母が0でないことが先です。等式の両辺を二乗する操作は、逆向きに戻せない場合があるため、得た候補を必ず元の式に代入します。"},
            {"type": "note", "variant": "warn", "content": "√(u+v)=√u+√v は一般には成り立ちません。u=v=1なら左辺√2、右辺2です。根号を外す法則には非負などの条件もあります。"},
            {"type": "example", "title": "例題：二つの距離の和", "body": "|x−1|+|x−4|=5を解け。", "answer": "1≤x≤4では距離の和は3で不適。x<1では5−2x=5よりx=0、x>4では2x−5=5よりx=5。両方を元の式に代入すると和が5になります。"},
            {"type": "example", "title": "例題：二乗で生じた余分な候補", "body": "√(x+2)=xを解け。", "answer": "左辺は非負なのでx≥0が必要。二乗してx²−x−2=0よりx=2,−1。−1は条件に反し元の式でも1≠−1。解はx=2だけです。"},
            {"type": "practice", "title": "練習：判断の理由も確かめよう", "problems": [{"body": "x<3のとき√((x−3)²)を簡単にせよ。", "answer": "|x−3|=3−x。根号の結果は非負なのでx−3にはなりません。"}, {"body": "1/(√3−√2)を有理化せよ。", "answer": "分子分母に√3+√2を掛けると分母は3−2=1。答えは√3+√2。"}, {"body": "|x+2|≤3を解け。", "answer": "−3≤x+2≤3より−5≤x≤1。−2から距離3以内の区間です。"}]},
      ...
      lessons: [
        {
          ...
          blocks: [
            {"type": "heading", "level": 3, "content": "単位を変えたとき、ばらつきはどう変わるか"},
            {"type": "text", "content": "全員の身長をcmからmに変えても、誰が高いかは変わりません。しかし数値のばらつきは100分の1になります。統計量を比較するときは単位と変換をそろえなければ、数値の大きさだけで結論を誤ります。"},
            {"type": "text", "content": "y=ax+bと変換すると平均もa倍してbを足した値になります。平均との差はy−平均y=a(x−平均x)なので、分散はa²倍、標準偏差は|a|倍です。加えるbは平均との差から消えるため、ばらつきに影響しません。"},
            {"type": "text", "content": "相関係数は直線的な関係を測ります。曲線上に規則正しく並んでいても0になることがあります。散布図を先に眺め、外れ値や曲がった形を確認してから係数を読む順番が大切です。以下の数列は教材用の架空データです。"},
            {"type": "note", "variant": "warn", "content": "ここでの分散はデータ全体の記述としてnで割ります。母分散を推定する不偏分散（n−1で割る）とは目的が違います。また、どちらかの標準偏差が0なら相関係数は定義できません。"},
            {"type": "example", "title": "例題：負の係数による変換", "body": "xの平均が4、分散が9のとき、y=−2x+10の平均・分散・標準偏差は。", "answer": "平均は−2×4+10=2、分散は(−2)²×9=36、標準偏差は√36=6。負の係数でもばらつきは負になりません。"},
            {"type": "example", "title": "例題：相関0でも規則はある", "body": "点(−1,1),(0,0),(1,1)の相関係数を求め、関係を説明せよ。", "answer": "x平均0、y平均2/3。偏差積の和は−1/3+0+1/3=0で、両分散は正なので相関係数0。一方、全点がy=x²を満たします。直線的な相関0は関係なしを意味しません。"},
            {"type": "practice", "title": "練習：判断の理由も確かめよう", "problems": [{"body": "データ2,4,6を10倍したときの分散と標準偏差は。", "answer": "元の分散8/3の100倍で800/3。標準偏差は10√(8/3)。偏差が10倍だから二乗の平均は100倍です。"}, {"body": "全データに−7を足すと分散はどうなるか。", "answer": "変わりません。平均も−7だけ移動し、各値と平均との差が変わらないからです。"}, {"body": "データがすべて5のとき、別のデータとの相関係数を0といえるか。", "answer": "いえません。このデータの標準偏差は0で、相関係数の分母が0となるため未定義です。"}]},
      ...
      lessons: [
        {
          ...
          blocks: [
            {"type": "heading", "level": 3, "content": "角度の倍率を、横軸の時間の伸び縮みとして読む"},
            {"type": "text", "content": "観覧車の高さは一周するたびに繰り返します。一周にかかる時間が半分なら、同じ時間に波が二つ現れます。sinの中のxを2xに置き換えるのは、このように進む角度を2倍速にすることです。"},
            {"type": "text", "content": "y=A sin(Bx+C)+D（A≠0、B≠0）では、中の角度が2π進むと一周期なので周期は2π/|B|。A≠0のとき振幅は|A|、中央の高さはDです。Bを括り出してB(x+C/B)とすると、横方向のずれは−C/Bと読めます。"},
            {"type": "text", "content": "三角方程式では、まず中の角度をtと置き、xの範囲からtの範囲も変換します。一周分だけで考えてしまうと、xを何倍かした場合に解を落とします。求めたtを元のxに戻すまでが一組の作業です。"},
            {"type": "note", "variant": "warn", "content": "振幅が0なら定数関数になり、最小の正の周期はありません。また角度をラジアンで扱う式に度の数値をそのまま入れないでください。180°はπラジアンです。"},
            {"type": "example", "title": "例題：波の式から最大最小を読む", "body": "y=3 sin(2x−π/2)+1の振幅・周期・最大値・最小値は。", "answer": "振幅3、周期π、中央の高さ1。sinは−1から1なので最大値4、最小値−2。2(x−π/4)より、y=3 sin(2x)+1を右へπ/4ずらした形です。"},
            {"type": "example", "title": "例題：二周分の解を拾う", "body": "0≤x<2πでsin(2x)=1/2を解け。", "answer": "t=2xと置くと0≤t<4π。t=π/6,5π/6,13π/6,17π/6なので、x=π/12,5π/12,13π/12,17π/12。範囲内に4個あります。"},
            {"type": "practice", "title": "練習：判断の理由も確かめよう", "problems": [{"body": "y=−2 cos(3x)+4の振幅・周期・値の範囲は。", "answer": "振幅2、周期2π/3、2≤y≤6。負号は波を上下反転しますが振幅は正です。"}, {"body": "0≤x<2πでcos x=−1/2を解け。", "answer": "x=2π/3,4π/3。単位円のx座標が−1/2になる第2・第3象限の2点です。"}, {"body": "y=sin(x+π/3)はy=sin xをどちらへ動かすか。", "answer": "左へπ/3。x=−π/3で中身が0となるため、原点にあった通過点が左へ移っています。"}]},
        ...
        },
        {
          ...
          blocks: [
            {"type": "heading", "level": 3, "content": "真数条件と単調性を先に確認する"},
            {"type": "text", "content": "対数は「何回掛けたか」を逆に尋ねる道具です。2を何乗すれば8になるかなら3。一方、底が1/2なら掛けるほど小さくなるので、同じ大小比較でも指数の大小は逆向きになります。"},
            {"type": "text", "content": "底a>1の対数は増加関数、0<a<1なら減少関数です。同じ底の対数を比較するとき、前者は真数の大小をそのまま、後者は逆にします。真数が正である条件は、解を出した後に足すのではなく最初に書きます。"},
            {"type": "text", "content": "指数式の中にa²ˣとaˣが並んでいるならt=aˣと置くと二次方程式になります。ただしt>0です。対数の和を積にまとめるときも、元の真数がそれぞれ正という条件を保持します。"},
            {"type": "note", "variant": "warn", "content": "log(x+y)は一般にlog x+log yではありません。積を和へ変える法則と混同しないこと。また底が0と1の間なら、対数を外す際に不等号の向きが逆になります。"},
            {"type": "example", "title": "例題：真数条件で候補を絞る", "body": "log₂(x−1)+log₂(x−3)=3を解け。", "answer": "真数条件からx>3。まとめると(x−1)(x−3)=8、すなわちx²−4x−5=0よりx=5,−1。条件に合うx=5のみ。元の式でもlog₂4+log₂2=3です。"},
            {"type": "example", "title": "例題：1より小さい底の不等式", "body": "log₍₁/₂₎(x−1)≥−2を解け。", "answer": "真数条件はx>1。底1/2では減少関数なので0<x−1≤(1/2)⁻²=4。したがって1<x≤5。右端では対数が−2となります。"},
            {"type": "practice", "title": "練習：判断の理由も確かめよう", "problems": [{"body": "4ˣ−5×2ˣ+4=0を解け。", "answer": "t=2ˣ>0と置くと(t−1)(t−4)=0。t=1,4はいずれも正なのでx=0,2。"}, {"body": "log₃(2x−1)<2を解け。", "answer": "真数条件x>1/2。底3は増加関数なので2x−1<9よりx<5。合わせて1/2<x<5。"}, {"body": "100×1.1ⁿ≥200を初めて満たす整数n≥0は。", "answer": "1.1ⁿ≥2よりn≥log 2/log 1.1≈7.27。最小整数は8。7回なら約194.87、8回なら約214.36です。"}]},
